...
| 技術可行性不確定 | `spike`、`spec-driven-development`、`source-driven-development` | 先用 throwaway experiment 或 source research，把未知拆成可驗證問題，再決定正式方案。 |
...
先掃描任務類型，再載入對應 skill。不要寫泛稱「某某 skill/resources」；必須對應到 `~/.copilot/skills/<skill-name>/SKILL.md` 內實際存在的 skill，或由 Copilot diagnostics 確認的其他支援位置。如果需要的 skill 不存在，要在 Skill Selection 中標成 missing，不可假裝已可用。
...
| 技術可行性不確定，需要 throwaway experiment | `spike`、`spec-driven-development`、`source-driven-development` | Step 0.5 / Step 6；實驗成果只作決策證據，不直接成為 production implementation |
...
`spike` 處理規則：只有在技術可行性存在具體未知時才選用；保持隔離、定義 timebox 與成功條件，並在正式實作前丟棄或重寫實驗程式碼。
...
6. Are any referenced skills missing from `~/.copilot/skills/<name>/SKILL.md` and Copilot diagnostics?
...
## 8. 現行 Agent Skill package

此 workflow 已封裝為 `~/.copilot/skills/frontend-task-preflight/SKILL.md`，並由 `~/.copilot/agents/frontend-task-preflight.agent.md` 提供具名入口與 subagent 編排。

Skill description 保持觸發條件導向，不要摘要完整流程，避免模型只看 description 不讀正文：

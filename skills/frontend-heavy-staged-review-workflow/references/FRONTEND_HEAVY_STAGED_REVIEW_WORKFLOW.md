...
3. Use real independent VS Code Chat subagents through the enabled `agent/runSubagent` tool.
...
11. Counted reviewers must read `~/.copilot/skills/<skill-name>/SKILL.md` or receive coordinator-injected skill criteria.
...
1. Do not select a skill unless `~/.copilot/skills/<skill-name>/SKILL.md` exists or Copilot diagnostics resolves it from another supported location.
...
Hard gate: confirm the selected VS Code custom agent has the `agent/runSubagent` tool. If no real sub-agent mechanism is available, stop and return `Incomplete`. Do not emulate sub-agents by writing multiple reviewer personas in one context.
...
| `skill_path` | `~/.copilot/skills/<selected_skill>/SKILL.md`; must exist. |
...
Use `templates/subagent-prompt.md` from this skill directory for the exact reviewer prompt and JSON schema. Required reviewer output fields include `run_id`, `reviewer_id`, `attempt`, `is_replacement`, `skill_used`, `skill_path`, `skill_read`, `skill_criteria_injected`, `status`, `failure_reason`, `angle`, `input_scope`, `verdict`, `confidence_summary`, `findings`, and `notes`.
...
The final report must be findings-only and written in Chinese. Use `templates/final-report.md` from this skill directory for the exact concise format.

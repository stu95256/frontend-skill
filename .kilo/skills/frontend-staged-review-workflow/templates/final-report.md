...
Omit empty severity sections. If there are no findings, output only `結論: Approve` and `已暫存 diff 中沒有發現問題。` The entire final user-facing report must be in Chinese, except code identifiers, paths, commands, enum values, and quoted source text. Do not include dispatch plans, reviewer ledgers, skill names, reviewer IDs, sub-agent provenance, or aggregation decision logs in the final user-facing report.
